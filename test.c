...
void leitura(struct evento *x, int contador){
   ...
   do {
    printf("Digite o hor�rio de in�cio (hora e minuto):\n");
...
do {
    printf("Digite o hor�rio de t�rmino (hora e minuto):\n");
    if (scanf("%d%d", &x->horario.hora2, &x->horario.min2) == 2 &&
        ...
        printf("Hor�rio inv�lido. Tente novamente. Certifique-se de que o hor�rio de t�rmino � maior que o hor�rio de in�cio.\n");
        while (getchar() != '\n');
    }
} while (!entradaValida);
...
void adicionarAmbiente(struct ambiente *ambientes, int *numAmbientes, struct evento *eventos, int numEventos) {
    if (*numAmbientes < MAX_AMBIENTES) {
        ...
        printf("Digite o n�mero do evento: ");
        scanf("%d", &numEvento);
        ...
        if (numEvento >= 1 && numEvento <= numEventos) {
            ...
            printf("N�mero de evento inv�lido! O ambiente foi adicionado, mas n�o associado a nenhum evento.\n");
            (*numAmbientes)++;
        }
    } else {
