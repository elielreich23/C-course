...
void mostra_Pessoa(struct Pessoa p) {
    printf("Nome: %s\nCPF: %s\nIdade: %d\nConjuge: %s\n conjuge nome:",
           p.nome, p.cpf, p.idade, p.conjuge.nome_conj, p.conjuge_nome);
}
...
void vincularConjuge(struct Pessoa *p1, struct Pessoa *p2) {
    ...
    strcpy(p1->conjuge.nome, p2->nome);
    strcpy(p2->conjuge.nome, p1->nome);


}

void listarPessoas(struct Pessoa *p, int n) {
    if (n == 0) {
